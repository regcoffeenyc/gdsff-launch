...
export default function RouteMetadata({ language }) {
  ...
  useEffect(() => {
    ...
    if (!meta) {
      // Never retain a previous page's indexing signals on an unknown route.
    ...
    for (const alternate of meta.alternates) {
      setTag(`link[rel="alternate"][hreflang="${alternate.language}"]`, 'link', {
        rel: 'alternate', hreflang: alternate.language, href: alternate.url,
      })
    }
